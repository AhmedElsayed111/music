...
async def sourc(client: Client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""✧ 𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝑻𝒐 𝑺𝒐𝒖𝒓𝒄𝒆 𝒓𝒊𝒕𝒂 ♪\n\n• ᴅᴇᴠᴇʟᴏᴘᴇʀ -› @UU_0B\n• ᴄʜᴀɴɴᴇʟ ᴍɪʀᴀ -› @UU_PATS\n\n**- للتنصيب او للاستفسار تواصل مع مطور السورس**""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                    ...
                    InlineKeyboardButton(
                       "تحديثات ميرا", url=f"https://t.me/UU_PATS")
...
async def aboutd5ev(client: Client, message: Message):
    usr = await client.get_chat(1918968211)
    name = usr.first_name
    bio = (await client.get_chat(1918968211)).bio
    async for photo in client.iter_profile_photos(1918968211, limit=1):
                    await message.reply_photo(photo.file_id, caption=f"""- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 𝑻𝒐 𝑩𝒐𝒕 𝒓𝒊𝒕𝒂 ♪ -› @UU_0B\n\n- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓'𝒔 𝑩𝒊𝒐 -› {bio}""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        name, user_id=1918968211)
                ],
            ]
        ),
    )
